...
Dr. Petru Andrei
Project 6: Lithium - Air Cathode Simulation
*/
#include "toolkit.hpp"
...
// #include "LiAir_gradSim.hpp"
double _DO2 (double porosity);
double _DLi (double porosity, double c_Li);
double _tplus (double c_Li);
double _sigma();
double _kappa (double porosity);
double _kappaD(double porosity, double c_Li);
...
int main()
{
    ...
    while(minporosity(x) > 0 && !nanFlag)
    {
        ...
        if(specCap <= 1.61)
        {   // total specific capacity is at 3.22
            fprintf(fp2, "%f, %f\n", specCap, phi[N]);
...
void ComputeFunct(vector < vector <double>> &rhs)
{
    ...
    for(int i = 1; i < N; i++)
        rhs[i][n] = _sigma() * (phi[i+1]+phi[i-1]-2*phi[i])/pow(h,2)
            + F*rrC(c_O2[i], porosity[i], phi_Li[i], phi[i]);
        
    rhs[N][n] = -_sigma()*A*(phi[N]-phi[N-1])/h - I;
    // ------------------Equation 1-------------------------------------------end               check
    
    // ------------------Equation 2-----------------------------------------start
    // electrostatic potential of Li ---- phi li                      (V)
    rhs[(N+1)][n] = -_kappa(porosity[0]) * (phi_Li[1]-phi_Li[0])/h + _kappaD(porosity[0],c_Li[0]) * (log(c_Li[1])-log(c_Li[0]))/h
        - F * rrA(phi_Li[0]);
        
    for(int i = 1; i < N; i++)
        rhs[(N+1)+i][n] = _kappa(porosity[i]) * (phi_Li[i+1]+phi_Li[i-1]-2*phi_Li[i])/pow(h,2)
            - _kappaD(porosity[i], c_Li[i]) * (log(c_Li[i+1])+log(c_Li[i-1])-2*log(c_Li[i]))/pow(h,2)
            - F * rrC(c_O2[i], porosity[i], phi_Li[i], phi[i]);
    
    rhs[(N+1)+N][n] = -_kappa(porosity[N]) * (phi_Li[N] - phi_Li[N-1])/h
        + _kappaD(porosity[N],c_Li[N]) * (log(phi_Li[N])-log(phi_Li[N-1]))/h;
    ...
    for(int i = 1; i < N; i++)
        rhs[2*(N+1)+i][n] = (porosity[i]*c_Li[i]-oporosity[i]*oc_Li[i])/tau
            - _DLi(porosity[i],c_Li[i]) * (c_Li[i+1]+c_Li[i-1]-2*c_Li[i])/pow(h,2)
            - (1-_tplus(c_Li[i])) * rrC(c_O2[i], porosity[i], phi_Li[i], phi[i]);
    ...
    for(int i = 1; i < N; i++)
        rhs[3*(N+1)+i][n] = (porosity[i]*c_O2[i]-oporosity[i]*oc_O2[i])/tau
            - _DO2(porosity[i]) *(c_O2[i+1]+c_O2[i-1]-2*c_O2[i])/pow(h,2)
...
void ComputeJacob(vector < vector <double>> &J)
{
    ...
    */
    ...
    for(int i = 1; i < N; i++)
    {
        J[i][i+1] = _sigma()/pow(h,2);
        J[i][i-1] = _sigma()/pow(h,2);
        J[i][i] = -2*_sigma()/pow(h,2) + F * drrC_dphi(c_O2[i], porosity[i], phi_Li[i], phi[i]);
    ...
    rhs[(N+1)+N][n] = -K * (phi_Li[N] - phi_Li[N-1])/h
        + K_D * (log(phi_Li[N])-log(phi_Li[N-1]))/h;
    */
    J[(N+1)][(N+1)] = _kappa(porosity[0])/h - F * drrA_dphiLi(phi_Li[0]);
    J[(N+1)][(N+1)+1] = -_kappa(porosity[0])/h;
    J[(N+1)][2*(N+1)] = -_kappaD(porosity[0], c_Li[0])/h*(1/c_Li[0]);
    J[(N+1)][2*(N+1)+1] = _kappaD(porosity[0], c_Li[0])/h*(1/c_Li[1]);
    
    for(int i = 1; i < N; i++)
    {
        J[(N+1)+i][i] = -F * drrC_dphi(c_O2[i], porosity[i], phi_Li[i], phi[i]);
        J[(N+1)+i][(N+1)+i+1] = _kappa(porosity[i])/pow(h,2);
        J[(N+1)+i][(N+1)+i-1] = _kappa(porosity[i])/pow(h,2);
        J[(N+1)+i][(N+1)+i] = -2*_kappa(porosity[i])/pow(h,2) - F * drrC_dphiLi(c_O2[i], porosity[i], phi_Li[i], phi[i]);
        J[(N+1)+i][2*(N+1)+i-1] = -_kappaD(porosity[i], c_Li[i])/pow(h,2) * (1/c_Li[i-1]);
        J[(N+1)+i][2*(N+1)+i+1] = -_kappaD(porosity[i], c_Li[i])/pow(h,2) * (1/c_Li[i+1]);
        J[(N+1)+i][2*(N+1)+i] = 2*_kappaD(porosity[i], c_Li[i])/pow(h,2) * (1/c_Li[i]);
    ...
    J[(N+1)+N][(N+1)+N] = -_kappa(porosity[N])/h;
    J[(N+1)+N][(N+1)+N-1] = _kappa(porosity[N])/h;
    J[(N+1)+N][2*(N+1)+N] = _kappaD(porosity[N], c_Li[N])/h * (1/c_Li[N]);
    J[(N+1)+N][2*(N+1)+N-1] = -_kappaD(porosity[N], c_Li[N])/h * (1/c_Li[N-1]);
    // ------------------Equation 2-------------------------------------------end               check
    ...
    */
    ...
    for(int i = 1; i < N; i++)
    {
        J[2*(N+1)+i][i] = -(1-_tplus(c_Li[i])) * drrC_dphi(c_O2[i], porosity[i], phi_Li[i], phi[i]);
        J[2*(N+1)+i][(N+1)+i] = -(1-_tplus(c_Li[i])) * drrC_dphiLi(c_O2[i], porosity[i], phi_Li[i], phi[i]);
        J[2*(N+1)+i][2*(N+1)+i-1] = -_DLi(porosity[i],c_Li[i])/pow(h,2);
        J[2*(N+1)+i][2*(N+1)+i+1] = -_DLi(porosity[i],c_Li[i])/pow(h,2);
        J[2*(N+1)+i][2*(N+1)+i] = porosity[i]/tau+2*_DLi(porosity[i],c_Li[i])/pow(h,2);
        J[2*(N+1)+i][3*(N+1)+i] = -(1-_tplus(c_Li[i])) * drrC_dcO2(c_O2[i], porosity[i], phi_Li[i], phi[i]);
        J[2*(N+1)+i][4*(N+1)+i] = c_Li[i]/tau-(1-_tplus(c_Li[i]))*drrC_dporosity(c_O2[i], porosity[i], phi_Li[i], phi[i]);
    }
    
    J[2*(N+1)+N][2*(N+1)+N] = 1;
    J[2*(N+1)+N][2*(N+1)+N-1] = -1;
    // ------------------Equation 3-------------------------------------------end               check
    ...
    */
    ...
    for(int i = 1; i < N; i++)
    {
        J[3*(N+1)+i][i] = 1/2 * drrC_dphi(c_O2[i], porosity[i], phi_Li[i], phi[i]);
        J[3*(N+1)+i][(N+1)+i] = 1/2 * drrC_dphiLi(c_O2[i], porosity[i], phi_Li[i], phi[i]);
        J[3*(N+1)+i][3*(N+1)+i-1] = -_DO2(porosity[i])/pow(h,2);
        J[3*(N+1)+i][3*(N+1)+i] = porosity[i]/tau + 2*_DO2(porosity[i])/pow(h,2)+ 1/2*drrC_dcO2(c_O2[i], porosity[i], phi_Li[i], phi[i]);
        J[3*(N+1)+i][3*(N+1)+i+1] = -_DO2(porosity[i])/pow(h,2);
        J[3*(N+1)+i][4*(N+1)+i] = c_O2[i]/tau + 1/2 * drrC_dporosity(c_O2[i], porosity[i], phi_Li[i], phi[i]);
    }
...
double _DO2 (double porosity)
{
    return D_O2 * pow(porosity, 1.5);
}
double _DLi (double porosity, double c_Li)
{
    return D_Li * exp(-0.357 * c_Li) * pow(porosity, 1.5);
}
double _tplus (double c_Li)
{
    return 0.4492 - 0.4717 * (c_Li/c_ref)
        + 0.4106 * (pow(c_Li,2)/pow(c_ref,2))
        - 0.1287 * (pow(c_Li,3)/pow(c_ref,3));
}
double _sigma()
{
    return sigma*pow(1-e0,1.5);
}
double _kappa (double porosity)
{
    return K * pow (porosity, 1.5);
}
double _kappaD(double porosity, double c_Li)
{
    return Vt*_kappa(porosity)*(1-_tplus(c_Li));
}