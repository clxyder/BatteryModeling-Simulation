...
Dr. Petru Andrei
Project 5: Lithium - Air Cathode Simulation
*/
#include "toolkit.hpp"
...
void ComputeJacob(vector < vector <double>> &J)
{
    ...
    */
    ...
    for(int i = 1; i < N; i++)
    {
        ...
        J[3*(N+1)+i][3*(N+1)+i] = porosity[i]/tau + 2*D_O2/pow(h,2)+ 1/2*drrC_dcO2(c_O2[i], porosity[i], phi_Li[i], phi[i]);
        J[3*(N+1)+i][3*(N+1)+i+1] = -D_O2/pow(h,2);
